...
function formatOrderToHTML(order: any): string {
  ...
  const serviceCharge = Number(order?.serviceCharge ?? order?.serviceCharges?.totalAll ?? 0);
  const tips = Number(order?.tips ?? 0);
  const deliveryFee = Number(order?.deliveryFee ?? 0);
  const discount = Number(order?.discountApplied?.discountAmount ?? order?.discount?.discountAmount ?? 0);
  ...
  <body>
    <div class="bill-container">
      ...
      <div class="order-time">
        ...
        ${order?.estimatedTimeToComplete ? `<div><b>ETA:</b> ${order.estimatedTimeToComplete} minutes</div>` : ''}
      </div>
      ...
      <div class="items">
        ${items.map(item => `
          <div class="item-row">
            <div class="item-name">${item.qty} × ${item.name} £${item.price.toFixed(2)}</div>
            ${item.attributes.map(attr => `
              <div class="item-attributes">
                ${attr.attributeName}: ${attr.selectedItems.map(si => si.itemName).join(', ')}
              </div>
            `).join('')}
      ...
      <div class="customer">
        ...
        ${customerEmail ? `<div class="customer-info"><b>Email:</b> ${customerEmail}</div>` : ''}
        ${customerPhone ? `<div class="customer-info"><b>Phone:</b> ${customerPhone}</div>` : ''}
      </div>
...
export async function printOrder(order: any, config?: PrintConfig): Promise<void> {
  try {
